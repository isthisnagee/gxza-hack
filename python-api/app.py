...
class Input(pydantic.BaseModel):
    ...
    store: bool = pydantic.Field(
        default=True, description="Whether to store the result in the database"
    )


class StructuredMedication(pydantic.BaseModel):
    name: str = pydantic.Field(
        description="The name of the medication. Fix typos if needed, for example, Prednsione should be corrected to Prednisone."
    )
    strength: str = pydantic.Field(
        description="The strength of the medication. e.g. 10mg"
    )
...
@app.post("/medication")
def medication(input_: StructuredMedication) -> StructuredMedication:
    db = firestore.client()
    db.collection("inventory").document().set(input_.model_dump())
    return input_


@app.post("/structured-medication")
def structured_medication(input_: Input) -> StructuredMedication:
    ...
    completion = client.beta.chat.completions.parse(
        ...
        tools=[openai.pydantic_function_tool(StructuredMedication)],
    )
    ...
    output = cast(StructuredMedication, tool_calls[0].function.parsed_arguments)

    if input_.store:
        db = firestore.client()
        db.collection("inventory").document().set(output.model_dump())
